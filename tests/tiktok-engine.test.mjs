...
state.people.push({id:'alt',type:'user',name:'另一个 USER 账号'});
state.activeUserAccountId='alt';state.currentUserId='alt';
m.ensureTikTok(state);
assert.equal(e.tikAutoDue(state),false);
m.tikPost(state,{authorId:'alt',kind:'story',title:'小号的第一条',caption:'刚刚到这里'});
assert.equal(e.tikAutoDue(state),true);
console.log('PASS: TikTok batch, media validation, world and account isolation, social actions, AI replies, live room, unique views, auto refresh');
