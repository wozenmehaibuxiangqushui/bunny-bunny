...
export function createApiSettingsRenderer({ store }) {
  ...
  function bind(container) {
    ...
    if (form.dataset.kind === "text") {
      ...
      container.querySelector('[data-image-enable-all]').onclick=()=>{form.enabled.checked=true;form.querySelectorAll('[data-image-channel]').forEach(input=>input.checked=true);showToast('已选择所有场景，点击「保存并应用」生效')};
      container.querySelector('[data-image-disable-all]').onclick=()=>{form.querySelectorAll('[data-image-channel]').forEach(input=>input.checked=false);showToast('已关闭各场景，点击「保存并应用」生效')};
