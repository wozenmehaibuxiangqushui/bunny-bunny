...
export function createApiSettingsRenderer({ store }) {
  ...
  function bind(container) {
    ...
    if (form.dataset.kind === "text") {
      ...
      form.addEventListener("submit", event => { event.preventDefault(); const data = Object.fromEntries(new FormData(form)); if(form.dataset.kind==="image")data.enabled=form.elements.enabled.checked;store.update(s => { s.mediaApis[form.dataset.kind] = { ...s.mediaApis[form.dataset.kind], ...data }; }); showToast(data.enabled?"生图已开启并保存":"生图配置已保存，CHAR 将使用文字图替代"); });
    }
  }
