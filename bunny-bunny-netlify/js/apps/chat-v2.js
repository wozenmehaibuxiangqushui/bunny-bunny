...
export function createChatRenderers({ store, navigate }) {
  ...
  function me(container) {
    ...
    container.innerHTML=`<button class="card user-profile-entry row" data-open-user>${initialsAvatar(user,{avatarUrl:user.avatarUrl||""})}<div class="meta"><strong>${escapeHtml(user.chatName||user.name)}</strong><span>${escapeHtml(user.signature||user.note||"点击头像编辑完整 USER 名片")}</span></div><span>›</span></button><section class="card user-profile-summary"><div class="setting-row"><div><span class="label">真实名字</span><small>${escapeHtml(user.name||"未填写")}</small></div><span class="pill">USER</span></div><div class="setting-row"><div><span class="label">所在地</span><small>${escapeHtml(user.location||user.city||"未填写")}</small></div></div><div class="setting-row"><div><span class="label">人物设定</span><small>${escapeHtml(user.personality||user.note||"未填写")}</small></div></div></section><div class="section-title"><h3>账号身份</h3><span>${(user.accounts||[]).length} 个</span></div><section class="card">${(user.accounts||[]).map((x,i)=>`<div class="setting-row"><div><span class="label">${i?"小号":"当前主号"}</span><small>${escapeHtml(x.name)}</small></div>${x.primary?'<span class="pill">使用中</span>':""}</div>`).join("")}<div class="setting-row"><div><span class="label">新建小号</span><small>身份资料与主 USER 分开</small></div><button class="button secondary" data-alt>＋ 添加</button></div></section>${chatTabs("me")}`;
    container.querySelector("[data-open-user]").onclick=()=>navigate("user-profile");
    container.querySelector("[data-alt]").onclick=()=>{const name=prompt("小号名称");if(!name)return;store.update(s=>personById(s,s.currentUserId).accounts.push({id:"acc-"+Date.now(),name,primary:false}));me(container)};
    bindChatTabs(container,navigate);
  }

  function openChatMenu(container){openSheet(`<div class="sheet-handle"></div><div class="sheet-title"><h3>发起</h3><button class="button ghost" data-sheet-close>关闭</button></div><div class="action-grid"><button class="action-card" data-action="group"><span class="action-icon">界</span>管理世界分组</button><button class="action-card" data-action="chat"><span class="action-icon">群</span>创建群聊</button><button class="action-card" data-action="friend"><span class="action-icon">＋</span>添加好友</button></div>`,{onReady(sheet){sheet.querySelector('[data-action="group"]').onclick=()=>{closeSheet();openGroupEditor(container)};sheet.querySelector('[data-action="chat"]').onclick=()=>showToast("请选择同一世界分组内的角色创建群聊");sheet.querySelector('[data-action="friend"]').onclick=()=>{closeSheet();navigate("add-friend")}}})}
...
function chatTabs(active){const icon=(path)=>`<svg viewBox="0 0 24 24" aria-hidden="true">${path}</svg>`;return `<nav class="chat-tabs"><button aria-label="聊天" data-chat-tab="chat" class="${active==="chats"?"active":""}">${icon('<path d="M4 5h16v11H9l-5 4V5z"/>')}</button><button aria-label="朋友圈" data-chat-tab="moments" class="${active==="moments"?"active":""}">${icon('<circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="2"/>')}</button><button aria-label="我" data-chat-tab="chat-me" class="${active==="me"?"active":""}">${icon('<circle cx="12" cy="8" r="3"/><path d="M5 20c1-5 13-5 14 0"/>')}</button></nav>`;}
