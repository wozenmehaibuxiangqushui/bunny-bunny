...
return (container,params={})=>{
 const personId=params.personId||"char-jun",state=store.getState(),person=personById(state,personId),user=personById(state,state.currentUserId),p=state.chatProfiles[personId],a=state.chatAppearance,langs=ttsLanguageOptions(state),mem=memoryProfile(state,personId);
 ...
 <div class="section-title"><h3>向量记忆与关系状态</h3><span>IndexedDB 本机存储</span></div>
 <section class="card stack memory-settings-card">
  <p class="callout">每轮回复前会检索关键词、相关长期记忆和当天纪念事件；总结在后台异步执行，不阻塞当前回复。</p>
  ...
  <label class="field"><span>阶段总结提示词</span><textarea name="regularMemoryPrompt" rows="4">${escapeHtml(mem.regularPrompt)}</textarea></label>
  <label class="field"><span>大总结提示词</span><textarea name="grandMemoryPrompt" rows="4">${escapeHtml(mem.grandPrompt)}</textarea></label>
  ${toggle("memoryDebugEnabled","提示词测试模式","保存每轮实际发送给对话 API 的完整隐藏提示词与记忆命中结果",mem.memoryDebugEnabled)}
  <button type="button" class="button secondary" data-memory-debug>打开角色内心 / 记忆调试页</button>
 </section>
 ...
 container.querySelector("[data-memory-debug]").onclick=()=>navigate("memory-debug",{personId,conversationId:params.conversationId});
 ...
 container.querySelector("[data-delete-memory]").onclick=()=>confirmAction("删除所有记忆","会清除聊天、动态及已保存的角色记忆，只保留 CHAR 人物设定和聊天配置。","全部删除",async()=>{await clearCharacterMemory(personId);store.update(s=>{for(const c of s.conversations.filter(x=>x.personId===personId)){s.messages[c.id]=[];c.preview="暂无消息";c.time=""}s.moments=(s.moments||[]).filter(x=>x.personId!==personId);s.friendRequests=(s.friendRequests||[]).filter(x=>x.personId!==personId&&x.fromPersonId!==personId);delete s.memoryProfiles?.[personId];for(const key of ["memories","characterMemories","longTermMemory"]){if(Array.isArray(s[key]))s[key]=s[key].filter(x=>x.personId!==personId&&x.characterId!==personId);else if(s[key]&&typeof s[key]==="object")delete s[key][personId]}});showToast("已删除与当前 CHAR 有关的记忆，仅保留人物设定")});
