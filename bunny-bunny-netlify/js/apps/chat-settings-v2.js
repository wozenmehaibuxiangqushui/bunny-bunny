...
function parseStickerLines(text){return String(text||"").split(/\r?\n/).map(line=>{const match=line.trim().match(/^(.*?)\s+(https?:\/\/\S+)\s*$/i);if(!match)return null;const description=match[1].trim()||"表情包";return{name:description.slice(0,28),description,url:match[2],tags:[description]}}).filter(Boolean)}
