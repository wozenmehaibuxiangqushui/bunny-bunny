...
return (container,params={})=>{
 ...
 <section class="card stack memory-settings-card">
  ...
  <button type="button" class="button secondary" data-character-diary>阅读角色日记</button>
 </section>
 ...
 <section class="card stack">
  ...
  <label class="field"><span>气泡大小 <output data-scale-out>${Math.round((a.bubbleScale||1)*100)}%</output></span><input name="bubbleScale" data-scale type="range" min=".7" max="1.6" step=".05" value="${a.bubbleScale||1}"></label>
  <label class="field"><span>圆角弧度 <output data-radius-out>${a.bubbleRadius?`${a.bubbleRadius}px`:'跟随样式'}</output></span><input name="bubbleRadius" data-radius type="range" min="0" max="36" step="1" value="${a.bubbleRadius??0}"></label>
 ...
 container.querySelector('[data-character-diary]').onclick=()=>navigate('character-diary',{personId});
 ...
 form.elements.bubbleRadius.oninput=e=>container.querySelector("[data-radius-out]").value=Number(e.target.value)?e.target.value+"px":"跟随样式";
...
function formAppearance(form,a){return{...a,backgroundOpacity:Number(form.elements.backgroundOpacity.value),interfaceCss:form.elements.interfaceCss.value,bubbleCss:form.elements.bubbleCss.value,bubbleColor:form.elements.bubbleColor.value,bubbleScale:Number(form.elements.bubbleScale.value),bubbleRadius:Number(form.elements.bubbleRadius.value),fontSize:Number(form.elements.fontSize.value),fontUrl:form.elements.fontUrl.value}}
