...
return (container,params={})=>{
 ...
 <div class="section-title"><h3>好友与聊天数据</h3><span>危险操作均需二次确认</span></div>
 ...
 function batchStickers(pid){openSheet(`<div class="sheet-title"><h3>导入 CHAR 表情包</h3><button type="button" class="button ghost" data-sheet-close>取消</button></div><label class="field"><span>图床地址与描述</span><textarea rows="7" data-char-sticker-urls placeholder="每行：名称 | 图片 URL | 内容描述"></textarea></label><button type="button" class="button" data-import-char-stickers>导入</button>`,{onReady(sheet){sheet.querySelector("[data-import-char-stickers]").onclick=()=>{const text=sheet.querySelector("[data-char-sticker-urls]").value,items=text.split(/\r?\n/).map(line=>{const p=line.split("|").map(x=>x.trim()),url=p.length>1?p[1]:p[0];return/^https?:\/\//.test(url)?{name:p[0]||"CHAR 表情",url,description:p[2]||p[0]||"CHAR 表情",tags:[p[2]||p[0]||"日常"]}:null}).filter(Boolean);if(!items.length)return showToast("没有有效的图片地址");store.update(s=>s.stickerLibraries.characters[pid].push(...items));closeSheet();showToast(`已导入 ${items.length} 个 CHAR 表情`)}}})}
 function localStickers(files,pid){files.slice(0,20).forEach(file=>readImage(file,url=>{const description=file.name.replace(/\.[^.]+$/,"");store.update(s=>s.stickerLibraries.characters[pid].push({name:file.name,url,description,tags:[description,"本地"]}))}));showToast("本地表情正在导入，文件名会作为默认描述")}
