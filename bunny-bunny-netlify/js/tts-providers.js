...
export function ensureTtsState(state){
  const saved=state.tts||{},savedProviders=saved.providers||{},legacy=state.voiceApis?.tts||{};
  state.tts={...TTS_DEFAULTS,...saved,providers:{}};
  for(const [id,defaults] of Object.entries(TTS_DEFAULTS.providers))state.tts.providers[id]={...defaults,...(savedProviders[id]||{}),...(legacy.provider===id?legacy:{})};
  return state.tts;
}

export function resolveTtsConfig(state,profile={}){const t=ensureTtsState(state),provider=t.activeProvider,c={provider,...t.providers[provider]};if(profile.voiceId)c.voiceId=c.voice=profile.voiceId;if(profile.voiceSpeed)c.speed=Number(profile.voiceSpeed);return c}
...
export async function synthesizeSpeech(config,text,{tone=""}={}){const clean=String(text||"").trim();if(!clean)return;const c=config||{},provider=c.provider||"browser",prosody=toneProsody(tone);if(provider==="browser")return browserSpeech(tone?{...c,speed:num(c.speed,1)*prosody.speed,pitch:num(c.pitch,1)*prosody.pitch}:c,clean);if(!c.apiKey)throw Error(`请先填写 ${TTS_PROVIDERS[provider]?.label||"TTS"} API Key`);let response;
  if(provider==="openai"||provider==="custom"){const instructions=[c.instructions,tone&&`请使用${tone}的语气朗读`].filter(Boolean).join("；");response=await fetch(`${trim(c.baseUrl)}${provider==="custom"?(c.endpoint||"/audio/speech"):"/audio/speech"}`,{method:"POST",headers:{Authorization:`Bearer ${c.apiKey}`,"Content-Type":"application/json",...extraHeaders(c.extraHeaders)},body:JSON.stringify({model:c.model,voice:c.voice,input:clean,response_format:c.format||"mp3",speed:num(c.speed,1),...(instructions?{instructions}:{})})})}
