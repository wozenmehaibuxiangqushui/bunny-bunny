...
export function resolveTtsConfig(state,profile={}){const t=ensureTtsState(state),provider=t.activeProvider,c={provider,...t.providers[provider]},voice=/^voice_kr_\d+$/i.test(profile.voiceId||"")?"":profile.voiceId,lang=languageCode(profile.language);if(voice){if(["minimax","elevenlabs"].includes(provider))c.voiceId=voice;else if(provider==="fish")c.referenceId=voice;else if(provider==="deepgram")c.model=voice;else c.voice=voice}if(profile.voiceSpeed)c.speed=Number(profile.voiceSpeed);if(lang){c.lang=lang;c.language=lang;c.languageCode=lang}if(profile.emotion&&profile.emotion!=="自动")c.emotion=mapEmotion(profile.emotion,profile.emotion);if(profile.toneStyle&&profile.toneStyle!=="自然")c.style=mapAzureStyle(profile.toneStyle);return c}
...
export function ttsPanel(state){const t=ensureTtsState(state),id=t.activeProvider,c=t.providers[id],models=c.models?.length?c.models:builtInModels(id),selected=c.model||c.voice||models[0]||"";return `<section class="tts-center">
  <p class="callout">这里只负责连接语音厂商。选择厂商、填写 Key、拉取并选择模型即可；音色、语言、情绪和语气在每个 CHAR 的聊天设置中调整。</p>
  <form class="stack card" data-kind="tts" data-provider="${id}"><label class="field"><span>TTS 厂商</span><select data-tts-provider>${Object.entries(TTS_PROVIDERS).map(([key,x])=>`<option value="${key}" ${key===id?"selected":""}>${x.label}</option>`).join("")}</select></label>
  ${connectionFields(id,c)}
  <div class="row"><label class="field" style="flex:1"><span>模型</span><select name="model" data-tts-model>${models.map(x=>`<option value="${escapeHtml(x)}" ${x===selected?"selected":""}>${escapeHtml(x)}</option>`).join("")||'<option value="">请先拉取模型</option>'}</select></label><button type="button" class="button secondary" data-fetch-tts-models ${id==="browser"?"disabled":""}>拉取模型</button></div>
  <div class="row"><button type="button" class="button secondary" data-test-tts>试听</button><button class="button">保存并启用</button></div></form></section>`}
...
export function bindTtsPanel(container,store,rerender){
  ...
  container.querySelector("[data-fetch-tts-models]")?.addEventListener("click",async()=>{try{saveForm(store,form);const state=store.getState(),id=form.dataset.provider,c=ensureTtsState(state).providers[id],catalog=await fetchTtsModels(id,c);store.update(s=>{const target=ensureTtsState(s).providers[id];target.models=catalog.models;target.languages=catalog.languages;if(!catalog.models.includes(target.model))target.model=catalog.models[0]||""});showToast(`已获取 ${catalog.models.length} 个模型和 ${catalog.languages.length} 种语言`);rerender()}catch(error){showToast(error.message)}});
  container.querySelector("[data-test-tts]").onclick=async()=>{try{saveForm(store,form);await synthesizeSpeech(resolveTtsConfig(store.getState()),"你好，这里是 Bunny Bunny 的语音测试。");showToast("试听完成")}catch(error){showToast(error.message)}};
}

function saveForm(store,form){const id=form.dataset.provider,data=Object.fromEntries(new FormData(form));store.update(s=>{const t=ensureTtsState(s),current=t.providers[id];t.activeProvider=id;t.providers[id]={...current,...data};if(id==="azure"||id==="google")t.providers[id].voice=data.model||current.voice})}
...
export async function fetchTtsModels(id,c){
  ...
  if(id==="elevenlabs"){models=items.filter(x=>x.can_do_text_to_speech!==false).map(x=>x.model_id||x.id);languages=items.flatMap(x=>(x.languages||[]).map(v=>v.language_id||v.code||v.name))}
  else if(id==="azure"){models=items.map(x=>x.ShortName);languages=items.flatMap(x=>[x.Locale,...(x.SecondaryLocaleList||[])])}
...
function builtInModels(id){return({browser:["system-default"],openai:["gpt-4o-mini-tts","tts-1","tts-1-hd"],minimax:["speech-2.8-hd","speech-2.8-turbo","speech-2.6-hd","speech-2.6-turbo"],elevenlabs:["eleven_multilingual_v2","eleven_flash_v2_5"],azure:["zh-CN-XiaoxiaoNeural"],google:["cmn-CN-Wavenet-A"],deepgram:["aura-2-thalia-en","aura-2-odysseus-en","aura-2-luna-en"],fish:["s2.1-pro-free","s2.1-pro","s2-pro"],custom:[]})[id]||[]}

export function ttsLanguageOptions(state){const t=ensureTtsState(state),id=t.activeProvider,c=t.providers[id],values=c.languages?.length?c.languages:builtInLanguages(id);return["自动",...new Set(values.map(languageLabel))]}
function builtInLanguages(id){const broad=["zh-CN","yue-HK","en-US","ja-JP","ko-KR","fr-FR","de-DE","es-ES","pt-BR","ru-RU","it-IT","ar-SA","hi-IN","th-TH","vi-VN","id-ID","tr-TR","nl-NL","pl-PL","uk-UA","ms-MY"];return({browser:browserLanguages(),openai:broad,minimax:broad,elevenlabs:broad,azure:broad,google:broad,deepgram:["en-US","es-ES"],fish:broad,custom:broad})[id]||broad}
function browserLanguages(){if(typeof speechSynthesis==="undefined")return["zh-CN","en-US"];return[...new Set(speechSynthesis.getVoices().map(x=>x.lang).filter(Boolean))]}
function languageLabel(code){return({"zh-CN":"中文","zh-TW":"繁體中文","yue-HK":"粤语","cmn-CN":"中文","en-US":"English","en-GB":"English (UK)","ja-JP":"日本語","ko-KR":"한국어","fr-FR":"Français","de-DE":"Deutsch","es-ES":"Español","pt-BR":"Português","ru-RU":"Русский","it-IT":"Italiano","ar-SA":"العربية","hi-IN":"हिन्दी","th-TH":"ไทย","vi-VN":"Tiếng Việt","id-ID":"Bahasa Indonesia","tr-TR":"Türkçe","nl-NL":"Nederlands","pl-PL":"Polski","uk-UA":"Українська","ms-MY":"Bahasa Melayu"})[code]||code}
function connectionFields(id,c){const f=(name,label,type="text",placeholder="")=>`<label class="field"><span>${label}</span><input name="${name}" type="${type}" value="${escapeHtml(c[name]||"")}" placeholder="${placeholder}" ${type==="password"?'autocomplete="off"':""}></label>`;if(id==="browser")return'<p class="callout">浏览器原生语音不需要 Key，可以直接保存使用。</p>';let html=f("apiKey","API Key","password","填写厂商 API Key");if(["openai","minimax","elevenlabs","google","deepgram","fish","custom"].includes(id))html=f("baseUrl","Base URL")+html;if(id==="minimax")html+=f("groupId","Group ID（国际新版账号可留空）","text","旧版/区域账号按控制台填写");if(id==="azure")html=f("region","Azure Region","text","例如 eastus")+html;if(id==="custom")html+=f("endpoint","合成路径","text","/audio/speech");return html}
...
function languageCode(value){return({"中文":"zh-CN","繁體中文":"zh-TW","粤语":"yue-HK",English:"en-US","English (UK)":"en-GB","日本語":"ja-JP",Français:"fr-FR","한국어":"ko-KR",Deutsch:"de-DE",Español:"es-ES",Português:"pt-BR",Русский:"ru-RU",Italiano:"it-IT","العربية":"ar-SA","हिन्दी":"hi-IN","ไทย":"th-TH","Tiếng Việt":"vi-VN","Bahasa Indonesia":"id-ID","Türkçe":"tr-TR",Nederlands:"nl-NL",Polski:"pl-PL","Українська":"uk-UA","Bahasa Melayu":"ms-MY"})[value]||(/^[a-z]{2,3}-[A-Z]{2}$/.test(value||"")?value:"")}
