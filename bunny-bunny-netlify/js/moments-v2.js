...
import { ensureAccountState, activeAccount, accountFriends, friendWorldGroup } from "./account-system.js";

const AUTO_CHECK_MS=90*1000;
...
function normalizeLegacyMoments(state){
  ...
  for(const post of state.moments||[]){post.accountId=post.accountId||main;post.groupId=post.groupId||friendWorldGroup(state,post.personId);post.createdAt=post.createdAt||Date.now()-60*60*1000;post.likes=post.likes||[];post.comments=post.comments||[]}
}
...
async function generatePosts(store,accountId,people,forced){
  const state=ensureAccountState(store.getState()),model=activeModel(state);if(!model)throw Error("请先在 API 设置中连接并启用文本模型");const account=personById(state,accountId),roster=people.map(person=>({id:person.id,name:person.name,type:person.type,age:person.age||"",occupation:person.occupation||"",persona:person.personality||person.persona||person.note||"",groupId:friendWorldGroup(state,person.id)}));
  const prompt=`生成社交软件朋友圈动态。当前 USER 账号：${account?.chatName||account?.name}。候选好友 JSON：${JSON.stringify(roster)}。${forced?"这是用户主动刷新，必须生成 1-3 条。":"判断最自然的一人是否此刻会发动态；可以不发。"}\n要求：严格贴合人物年龄、职业和性格；像真人随手分享，不写舞台动作，不提 AI 或设定；每条仅属于 authorId 对应的 groupId；同一世界观才可互动。严格只返回 JSON：{"posts":[{"authorId":"候选id","text":"动态正文","comments":[{"personId":"同组候选id","text":"自然评论"}],"likes":["同组候选id"]}]}。`;
  ...
  store.update(s=>{for(const item of posts){const author=allowed.get(item.authorId),sameGroup=new Set(roster.filter(x=>x.groupId===author.groupId).map(x=>x.id));s.moments.unshift({id:crypto.randomUUID(),accountId,personId:item.authorId,groupId:author.groupId,text:String(item.text||"").slice(0,600),createdAt:Date.now(),likes:(item.likes||[]).filter(id=>sameGroup.has(id)).map(personId=>({personId})),comments:(item.comments||[]).filter(x=>sameGroup.has(x.personId)).map(x=>({personId:x.personId,text:String(x.text||"").slice(0,180)})),aiGenerated:true})}})
}
async function generateReactions(store,postId){
  ...
  const prompt=`USER ${account?.chatName||account?.name} 刚发布朋友圈：“${post.text}”。只有以下同世界观好友看得见：${JSON.stringify(people.map(x=>({id:x.id,name:x.name,persona:x.personality||x.persona||x.note||""})))}。根据每个人性格决定谁点赞、谁评论，允许有人不互动。严格只返回 JSON：{"likes":["id"],"comments":[{"personId":"id","text":"自然短评论"}]}。`;
  try{const data=parseJson(await sendToModel(model,[{role:"user",text:prompt}],"")),allowed=new Set(people.map(x=>x.id));store.update(s=>{const target=s.moments.find(x=>x.id===postId);if(!target)return;target.likes=[...(target.likes||[]),...(data?.likes||[]).filter(x=>allowed.has(x)).map(personId=>({personId}))];target.comments=[...(target.comments||[]),...(data?.comments||[]).filter(x=>allowed.has(x.personId)).map(x=>({personId:x.personId,text:String(x.text||"").slice(0,180)}))];target.aiInteracted=true})}catch{}
}
