...
import { ensureDailySchedule, enrichDailySchedule, currentSchedule, schedulePrompt } from "./schedule-engine.js";
...
export function createMomentsRenderer({store,navigate}){
  return function render(container){
    if(!container.__momentsRefreshBound){container.__momentsRefreshBound=true;container.addEventListener('bunny:moments-refresh',()=>{if(container.isConnected&&container.closest('[data-app=moments]'))render(container)})}
    ...
    container.querySelectorAll("[data-moment-image-retry]").forEach(button=>button.onclick=async()=>{button.disabled=true;button.textContent="正在生成…";try{await generateMomentPhoto(store,button.dataset.momentImageRetry)}catch(error){showToast(error.message)}render(container)});
...
function postCard(post,state,account){
  ...
  return `<article class="moment-card-v2"><aside>${initialsAvatar(author,profile)}</aside><div class="moment-main"><header><strong>${escapeHtml(author.chatName||profile.remark||author.name)}</strong><span>${relativeTime(post.createdAt)}</span></header><p class="moment-text">${escapeHtml(post.text||"")}</p>${post.image?`<img class="moment-image" src="${escapeHtml(post.image)}" alt="朋友圈图片">`:post.imageGenerating?'<div class="moment-image-loading" role="status" aria-label="正在生成朋友圈图片"><i></i><i></i><i></i></div>':post.imageError?`<button type="button" class="moment-image-retry" data-moment-image-retry="${post.id}">图片暂未生成 · 轻点重试</button>`:""}<div class="moment-toolbar"><button class="${liked?"active":""}" data-moment-like="${post.id}">${liked?"♥":"♡"} ${likes.length||""}</button><button data-moment-comment="${post.id}">评论</button><span>${escapeHtml(groupName(state,post.groupId))}</span></div>${likes.length||comments.length?`<section class="moment-social-v2">${likes.length?`<div class="moment-likes">♥ ${likes.map(item=>escapeHtml(actorName(item,state))).join("、")}</div>`:""}${comments.map(item=>`<p class="${item.replyTo?"moment-reply":""}"><b>${escapeHtml(actorName(item,state))}</b>${item.replyTo?`<span> 回复 ${escapeHtml(commentAuthor(post,item.replyTo,state))}</span>`:""} ${escapeHtml(item.text||"")}</p>`).join("")}</section>`:""}</div></article>`;
}
...
async function generatePosts(store,accountId,people,forced){
  for(const person of people){ensureDailySchedule(store,person.id);void enrichDailySchedule(store,person.id)}
  const state=ensureAccountState(store.getState()),model=activeModel(state);if(!model)throw Error("请先在 API 设置中连接并启用文本模型");const account=personById(state,accountId),roster=await Promise.all(people.map(person=>momentActorContext(state,accountId,person,store))),imageAllowed=Boolean(state.mediaApis?.image?.enabled&&state.mediaApis.image.channels?.moments);
  const prompt=`生成社交软件朋友圈动态。当前 USER 账号：${account?.chatName||account?.name}。候选好友资料、此刻时间、相关记忆与最近聊天 JSON：${JSON.stringify(roster)}。${forced?"这是用户主动刷新，必须生成 1-3 条。":"判断最自然的一人是否此刻会发动态；可以不发。"}\n要求：不能只照抄人物设定，必须结合记忆、最近聊天、当前时间、职业日程和当下关系来决定发不发以及发什么；动态要像真人随手分享，允许与刚聊过的事、未完成计划或近期心情自然相关，但不要复述记忆档案。严格贴合年龄、性格和线上表达习惯，不写舞台动作，不提 AI 或设定。每条仅属于 authorId 对应的 groupId；评论与点赞者必须同一世界观。评论者可对作者或别人的评论留下可继续回复的空间。严格只返回 JSON：{"posts":[{"authorId":"候选id","text":"动态正文","comments":[{"personId":"同组候选id","text":"自然评论"}],"likes":["同组候选id"],"image":${imageAllowed?'true或false':'false'},"imageDescription":"画面内容中文短句或空字符串"}]}。如 image=true，则 imageDescription 只描述画面，不要生图模型提示词；没图填 false。`;
  ...
  const imageJobs=[];store.update(s=>{for(const item of posts){const author=allowed.get(item.authorId),sameGroup=new Set(roster.filter(x=>x.groupId===author.groupId).map(x=>x.id)),comments=(item.comments||[]).filter(x=>sameGroup.has(x.personId)).map(x=>({id:crypto.randomUUID(),personId:x.personId,text:String(x.text||"").slice(0,180),createdAt:Date.now()}));const postId=crypto.randomUUID(),needsImage=imageAllowed&&item.image===true&&String(item.imageDescription||item.text||"").trim();if(needsImage)imageJobs.push({id:postId,description:String(item.imageDescription||item.text).slice(0,360),authorId:item.authorId});s.moments.unshift({id:postId,accountId,personId:item.authorId,groupId:author.groupId,text:String(item.text||"").slice(0,600),imageGenerating:Boolean(needsImage),imageDescription:needsImage?String(item.imageDescription||item.text).slice(0,360):"",createdAt:Date.now(),likes:(item.likes||[]).filter(id=>sameGroup.has(id)).map(personId=>({personId})),comments,aiGenerated:true,replyDueAt:comments.length?Date.now()+15000:0,threadProcessedCommentCount:0})}});
  await Promise.allSettled(imageJobs.map(job=>generateMomentPhoto(store,job.id)));

}
async function generateMomentPhoto(store,postId){
  const state=store.getState(),post=state.moments.find(x=>x.id===postId);if(!post)throw Error('动态不存在');if(!state.mediaApis?.image?.enabled||!state.mediaApis.image.channels?.moments)throw Error('请先在生图设置开启朋友圈场景');
  store.update(s=>{const item=s.moments.find(x=>x.id===postId);item.imageGenerating=true;item.imageError=''});
  try{const actor=personById(state,post.personId),profile=state.chatProfiles?.[post.personId]||{},prompt=`一张属于 ${actor?.name||'角色'} 的真实生活动态照片：${post.imageDescription||post.text}。人物职业和日常：${actor?.occupation||''} ${actor?.personality||actor?.persona||''}。不要加入文字或水印。`,src=await generateImage(state.mediaApis.image,{prompt,referenceImage:profile.imageReferenceFace||''}),response=await fetch(src);if(!response.ok)throw Error('图片返回失败');const media=await saveMediaBlob(await response.blob());store.update(s=>{const item=s.moments.find(x=>x.id===postId);if(item){item.image=media.src;item.imageMediaId=media.mediaId;item.imageGenerating=false}})}catch(error){store.update(s=>{const item=s.moments.find(x=>x.id===postId);if(item){item.imageGenerating=false;item.imageError=error.message}});throw error}finally{document.querySelector('#app-screen[data-app=moments] #app-view')?.dispatchEvent(new Event('bunny:moments-refresh'))}
}
async function generateReactions(store,postId){
  ...
  const contexts=await Promise.all(people.map(person=>momentActorContext(state,post.accountId,person,store))),prompt=`USER ${account?.chatName||account?.name} 刚发布朋友圈：“${post.text}”。只有以下同世界观好友看得见；资料包含各自相关记忆和最近聊天：${JSON.stringify(contexts)}。结合人物性格、记忆、与 USER 的近期互动决定谁点赞、谁评论，允许有人不互动。评论必须像本人，不要把记忆直接复述出来。严格只返回 JSON：{"likes":["id"],"comments":[{"personId":"id","text":"自然短评论"}]}。`;
...
async function generateThreadReplies(store,postId){
  ...
  const contexts=await Promise.all(people.map(person=>momentActorContext(state,post.accountId,person,store))),commentRows=(post.comments||[]).map(x=>({id:x.id,personId:x.personId,name:actorName(x,state),replyTo:x.replyTo||"",text:x.text})),prompt=`为一条朋友圈生成自然的楼中楼回复。动态作者 ${actorName({personId:post.personId},state)}，正文：“${post.text}”。同世界观人物及其记忆/最近聊天：${JSON.stringify(contexts)}。现有评论：${JSON.stringify(commentRows)}。只处理最后 ${pending.length} 条尚未处理的评论。作者本人和其他同世界观 CHAR/NPC 都可能回复别人；是否回复、谁回复、回复几条取决于各自性格、关系、记忆与评论内容，允许零条。不得让不同世界观的人互相看见；不要复述档案。严格只返回 JSON：{"replies":[{"responderId":"人物id","replyTo":"被回复评论id","text":"自然回复"}]}。`;
...
async function momentActorContext(state,accountId,person,store){
  ensureDailySchedule(store,person.id);
