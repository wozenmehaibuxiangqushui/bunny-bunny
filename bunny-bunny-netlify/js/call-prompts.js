export const DEFAULT_CALL_PROMPTS={
  ...
  chatActions:`你可以正常聊天，也可以在剧情合理时发送特殊消息。若要给用户转账，单独输出 [[TRANSFER:金额:备注]]；若要发红包，单独输出 [[REDPACKET:金额:备注]]，红包金额不得超过 520。除此之外不要输出这些标记。图片或表情信息会以描述文本提供；角色未开启识图时，只依据描述理解。`
};
...
export function extractChatActions(text){const actions=[];const clean=String(text||"").replace(/\[\[(TRANSFER|REDPACKET):([\d.]+):([^\]]*)\]\]/g,(_,kind,amount,note)=>{actions.push({kind:kind==="TRANSFER"?"transfer":"redpacket",amount:Number(amount),note:note.trim()});return""}).trim();return{clean,actions}}
