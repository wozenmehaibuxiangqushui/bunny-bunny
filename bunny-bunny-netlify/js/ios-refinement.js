...
export function setupIosRefinement({store,navigate}){
 ...
 screen.addEventListener("pointerup",e=>{if(!swipe)return;const dx=e.clientX-swipe.x,dy=Math.abs(e.clientY-swipe.y);swipe=null;if(dx>72&&dy<55){const r=getRoute();if(r.name==="chat-settings"&&r.params.conversationId)navigate("conversation",{id:r.params.conversationId});else if(r.name==="call"&&r.params.id)navigate("conversation",{id:r.params.id});else navigate(r.name==="conversation"?"chat":"desktop");showToast("已退出当前界面")}}, {passive:true});
