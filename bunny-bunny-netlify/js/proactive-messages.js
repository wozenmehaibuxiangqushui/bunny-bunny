  ...
  const check=async()=>{
    ...
    try{
      ...
      for(const conv of state.conversations){
        ...
        const idle=last&&now-Number(last.createdAt||0)>90*60*1000;
        ...
        try{const raw=await sendToModel(model,[{role:"user",text:prompt}],""),result=parseJson(raw);if(!result?.send||!result.text)continue;const time=timeNow(),id=crypto.randomUUID();store.update(s=>{(s.messages[conv.id]||(s.messages[conv.id]=[])).push({id,role:"char",type:"text",text:String(result.text),translation:result.translation||"",time,createdAt:Date.now()});const target=s.conversations.find(x=>x.id===conv.id);target.preview=result.text;target.time=time;target.unread=Number(target.unread||0)+1;s.callRuntime.lastProactiveAt[conv.id]=Date.now()})}catch{}
        break;
      }
