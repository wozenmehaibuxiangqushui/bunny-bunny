  ...
  const check=async()=>{
    ...
    try{
      ...
      for(const conv of conversationsForAccount(state)){
        const profile=state.chatProfiles[conv.personId]||{};if(!profile.proactive||inQuietHours(profile.quietHours,effectiveNow(profile)))continue;
        ...
        const prompt=`你是${person.name}，人物设定：${person.personality||person.persona||person.note||""}。USER 是${user.name}，资料：${user.personality||user.note||""}。\n${timeContext}\n${waiting?"USER 已读了你上一条消息但暂时没有回复。只有活泼、爱撒娇或较幼稚的人设才可以自然追问；成熟克制的人设应选择不发送。":"你们已经一段时间没有聊天，请结合此刻时间、生活安排和关系判断是否会主动联系。"}\n最近对话：\n${recent}\n必须像人物本人发消息，贴合其年龄、性格、关系与线上表达习惯，不写动作描写、AI 套话或强行追问。严格只返回 JSON：{"send":true或false,"text":"若发送则是自然短消息","translation":"外语消息的中文翻译，否则留空"}。不要解释。`;
