...
export function createConversationV3Renderer({ store, navigate }) {
  ...
  function sendRich(conv,data,container,params){store.update(s=>{s.messages[conv.id].push({id:id(),role:"user",time:timeNow(),...data});const target=s.conversations.find(item=>item.id===conv.id);if(target){target.preview=data.description||data.text||"新消息";target.time=timeNow()}});ui.attachmentsOpen=false;activeRender(container,params)}
