...
export function createConversationV3Renderer({ store, navigate }) {
  const ui = { conversationId:"", quoteId: "", selectMode: false, selected: new Set(), attachmentsOpen:false, sending:false, error:"" };
  ...
  function bind(container, params, conv, person, user, profile, messages) {
    ...
    container.querySelectorAll("[data-play-voice]").forEach(button=>button.onclick=()=>playVoiceMessage(messages.find(x=>x.id===button.dataset.playVoice),profile,button));
  ...
  async function submitMessage(sendAi,form,conv,person,profile,container,params){
    ...
    const input=form.elements.message,text=input.value.trim();if(!text)return showToast("先输入一条消息，再发送给 AI");
    ...
    ui.quoteId="";ui.error="";input.value="";
    if(sendAi)ui.sending=true;
    activeRender(container,params);
    if(!sendAi)return;
    await requestAiReply(conv,person,profile,container,params);
  }

  async function requestAiReply(conv,person,profile,container,params){
    ...
    try{
      ...
      for(const action of parsed.actions){if(action.kind==="sticker"){const query=action.query.toLowerCase(),item=charStickers.find(x=>[x.name,x.description,...(x.tags||[])].filter(Boolean).some(v=>String(v).toLowerCase().includes(query)||query.includes(String(v).toLowerCase())))||charStickers[0];if(item)store.update(s=>s.messages[conv.id].push({id:id(),role:"char",type:"sticker",text:item.name||"表情包",src:item.url,description:item.description||(item.tags||[]).join("、")||"CHAR 发送的表情包",time:timeNow()}));continue}if(action.kind==="voice"){store.update(s=>s.messages[conv.id].push({id:id(),role:"char",type:"voice",text:action.text,tone:action.tone,duration:estimateVoiceDuration(action.text),time:timeNow()}));continue}if(action.kind==="reaction"){store.update(s=>{const target=findActionTarget(s.messages[conv.id],action.target,"user");if(target){target.reaction=action.emoji;target.reactionBy="char"}});continue}if(action.kind==="recall"){store.update(s=>{const target=findActionTarget(s.messages[conv.id],action.target,"char");if(target&&!target.recalled){target.recalled=true;target.recalledText=target.text;target.recalledAt=Date.now()}});continue}const kind=action.kind==="redpacket"&&action.amount>520?"transfer":action.kind,messageId=id();store.update(s=>s.messages[conv.id].push({id:messageId,role:"char",type:kind,text:action.note||(kind==="redpacket"?"大吉大利":"转账给你"),amount:action.amount,time:timeNow()}));walletCredit(store,{amount:action.amount,kind,title:`收到 ${person.name} 的${kind==="redpacket"?"红包":"转账"}`,conversationId:conv.id,messageId,note:action.note})}
      activeRender(container,params);
  ...
  function handleFoldAction(action,conv,person,container,params){
    ...
    if(action==="album"||action==="camera"){const input=document.createElement("input");input.type="file";input.accept="image/*";if(action==="camera")input.capture="environment";input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{const src=await readImageOptimized(file);sendRich(conv,{type:"image",text:file.name,src,description:`用户发送的照片：${file.name}`},container,params)}catch(error){showToast(error.message)}};input.click();return}
  ...
  async function playVoiceMessage(message,profile,button){if(!message||button.classList.contains("playing"))return;button.classList.add("playing");try{if(message.audioUrl){const audio=new Audio(message.audioUrl);await audio.play();await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=()=>reject(Error("语音文件无法播放"))})}else await synthesizeSpeech(resolveTtsConfig(store.getState(),profile),message.text,{tone:message.tone||""})}catch(error){showToast(error.message)}finally{button.classList.remove("playing")}}
...
function messageContent(message){
  ...
  if(message.type==="voice")return`<button class="voice-message" data-play-voice="${message.id}" type="button"><span class="voice-wave"><i></i><i></i><i></i><i></i><i></i></span><strong>${formatDuration(message.duration||estimateVoiceDuration(message.text))}</strong><small>${escapeHtml(message.transcript||message.text||"语音消息")}</small></button>`;
...
function composerView(quote,open,sending,error){const actions=[["camera","camera","拍摄"],["album","image","照片"],["voice","mic","语音"],["text-image","text","文字图"],["redpacket","gift","红包"],["transfer","money","转账"],["sticker","smile","表情包"],["call","phone","语音通话"],["video","video","视频通话"]];return `<div class="composer-shell-v3 ${open?"attachments-open":""}">${error?`<div class="composer-error"><span>${escapeHtml(error)}</span><button type="button" data-open-api>去配置模型</button></div>`:""}${quote?`<div class="composer-quote"><div><strong>回复</strong><span>${escapeHtml(quote.text)}</span></div><button data-cancel-quote aria-label="取消引用">×</button></div>`:""}<div class="folded-attachments"><div>${actions.map(x=>`<button type="button" data-fold-action="${x[0]}">${foldIcon(x[1])}<span>${x[2]}</span></button>`).join("")}</div></div><form class="composer-v3" data-composer><button type="button" data-plus aria-label="展开更多功能">${open?"×":"＋"}</button><textarea name="message" rows="1" placeholder="iMessage" aria-label="消息"></textarea><button type="button" class="send-icon" data-send-only aria-label="仅发送">${sendIcon()}</button><button type="button" class="ai-send-v3 ${sending?"sending":""}" data-send-ai aria-label="发送并让 AI 回复" ${sending?"disabled":""}>${sending?'<i></i>':sparkIcon()}</button></form></div>`}
...
function menuIcon(type){const d={quote:'M7 11h11a5 5 0 0 1 5 5v3M7 11l4-4m-4 4 4 4',edit:'M5 21l4-.8L21 6.2 17.8 3 4 16.2 5 21Z',multi:'M8 7h14v14H8zM3 3h14v4M3 3v14h5',recall:'M8 8 4 12l4 4M5 12h10a6 6 0 0 1 6 6v2',delete:'M6 8h14M10 8V5h6v3m-8 0 1 14h8l1-14'}[type]||"M5 13h16";return`<svg viewBox="0 0 26 26" aria-hidden="true"><path d="${d}"/></svg>`}
